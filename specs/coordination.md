...
| `corrupt_project_config` | No — the feed still renders; one warning per invocation. D1 diagnostics are disabled for this invocation (D1 consumes `bead.id_pattern`). D6 is unaffected and continues to run. | Diagnostics family config-load layer (`internal/kerftranscript/`) — single emission from the shared layer, not per-detector. | `project.yaml`'s `bead.id_pattern` fails to compile as a regex. See [diagnostics.md §Bead-ID resolution](diagnostics.md#bead-id-resolution). |
