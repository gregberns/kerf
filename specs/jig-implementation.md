...
The implement pass (Pass 3) is a process pass — its state lives in the bead database (`bd`), not in work directory files. `kerf show` reports bead status for implementation works. See [jig-system.md](jig-system.md) §Process Passes vs. Artifact Passes.
