...
- **`priority_inversions`** — count of dispatch events where the selected bead is new-work and at least one rework bead was queue-eligible (dependencies met, not in-progress) at the moment of dispatch. No arrival-tick comparison is applied: the metric counts contention events where the policy faced a new-work-vs-rework choice and picked new-work, regardless of which arrived first. (Earlier drafts required the rework bead to have a lower arrival tick than the new-work bead; this was structurally unreachable because initial new-work is seeded with `arrival_tick = 0` while rework arrives at `arrival_tick >= 1`, so the inequality could never hold. See `plans/008_exploratory_testing/sim_scenarios/diagnosis.md`.)
