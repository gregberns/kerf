...
- **`top_of_queue_churn`** — after every event that mutates the bead store (dispatch, arrival, completion), the top-ranked bead is computed. Churn is the count of such events where the top changed divided by the total number of mutating events, evaluated inside the post-warmup window. The first mutating event establishes the initial top; the numerator counts changes starting from the second mutating event onward. **Single-candidate snapshots are excluded from both numerator and denominator.** A snapshot is "single-candidate" when fewer than two beads are queue-eligible (dependencies met, not in-progress) at the moment the top is computed. The metric measures policy-induced oscillation between competing choices; events at which the ranking function had no alternative carry no signal and must not dilute the ratio. Equivalently: an event where the head shifts solely because the prior head left the eligible set (completion or dispatch in a linear chain) is not "churn" if no other bead was eligible at that instant.
...
- **`priority_inversions`** — count of dispatch events where the selected bead is new-work and at least one rework bead was queue-eligible (dependencies met, not in-progress) at the moment of dispatch. No arrival-tick comparison is applied: the metric counts contention events where the policy faced a new-work-vs-rework choice and picked new-work, regardless of which arrived first. (Earlier drafts required the rework bead to have a lower arrival tick than the new-work bead; this was structurally unreachable because initial new-work is seeded with `arrival_tick = 0` while rework arrives at `arrival_tick >= 1`, so the inequality could never hold. The spec was updated to drop the arrival-tick gate; the implementation in `internal/sim/metrics/hooks.go` (`reworkEligibleAtDispatch`) was aligned with the updated spec under Plan 011 / pillar E. See `plans/008_exploratory_testing/sim_scenarios/diagnosis.md` for the original diagnosis and `internal/sim/run/run_test.go:TestRun_BaselineRandom_ProducesInversions` for the regression test.)
