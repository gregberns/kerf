...
> Built-in jig for planning changes to existing codebases. Write a plan before changing code.

This spec defines the `plan` jig that ships with kerf. It is the primary jig for existing projects where code is the source of truth and changes should be planned before implementation. See [jig-system.md](jig-system.md) for jig file format, resolution, versioning, and the review pattern. See [jig-spec.md](jig-spec.md) for spec-first projects. See [jig-bug.md](jig-bug.md) for defect investigation.

## When To Use

The `plan` jig applies when:

- You have an existing codebase and want to plan a change before implementing it
- The work involves designing something new or substantially changing existing behavior
- The problem must be understood before a solution can be specified
- Multiple components or subsystems are involved
- Code is the source of truth — the plan is a change document, not a living spec

It does not apply to:

- Defects (use the [`bug`](jig-bug.md) jig)
- Greenfield projects where specs are the source of truth (use the [`spec`](jig-spec.md) jig)
- Changes where the solution is already fully understood and requires no decomposition
...
Read the existing codebase and document the current state of the areas that will be affected by the change. This pass is unique to the plan jig — the spec jig does not need it because there is no existing codebase to map.
...
2. Explore the codebase systematically. For each area affected by the change:
   ...
   - Note architectural patterns in use (e.g., "this module uses the repository pattern", "error handling follows X convention").
...
Investigate technical approaches, existing patterns, and risks for each component. The `{component}` placeholder expands to one directory per component identified in Pass 3.
...
3. **Delegate research to a sub-agent.** For each component (or batch of components), spawn a sub-agent with fresh context. Provide:
   - The component's requirements from `03-components.md`
   - The relevant section of `02-analysis.md`
   - The research questions
   - Access to the codebase
   The sub-agent explores the codebase, checks external docs/APIs, identifies technical constraints, and returns findings.
...
Write implementation-level change specifications for each component, informed by the research findings. The `{component}` placeholder expands to one file per component identified in Pass 3.
...
5. Address migration and backwards compatibility if applicable. If this changes existing behavior, how does the transition work?
...
Up to 3 review rounds. Save findings to `change-spec-review.md`. After the final round, present artifacts and any unresolved findings to the user for approval. In autonomous mode: if no findings remain, advance. If unresolved findings remain after max rounds, advance with findings saved under an `## Unresolved` section in `change-spec-review.md`.
...
Up to 3 review rounds. Save findings to `integration-review.md`. After the final round, present artifacts and any unresolved findings to the user for approval. In autonomous mode: if no findings remain, advance. If unresolved findings remain after max rounds, advance with findings saved under an `## Unresolved` section in `integration-review.md`.

### Pass 7: Tasks (tasks)

**Output:** `07-tasks.md`

Break the spec into implementation tasks with dependencies. Each task is a unit of work an implementer can pick up independently.

#### Agent Instructions

You are producing a task list that makes the assembled spec actionable. The task list is implementation-agnostic — it works with any tracker or workflow tool.
...
2. Break the spec into implementation tasks. Each task specifies:
   ...
   - **Acceptance criteria** — how to verify the task is complete (drawn from the component spec's acceptance criteria)
...
4. Include integration tasks — these are tasks that wire components together, as defined in `06-integration.md`.
5. Include test tasks — these may be per-component or integration-level.
6. Verify completeness: every section of `SPEC.md` must be covered by at least one task. Every acceptance criterion from every component spec must appear in at least one task's acceptance criteria.
7. **Save to disk:** Write `07-tasks.md`.
8. Run the review process (see Review Criteria below).
9. After review completes, advance status to `ready`.
...
- `07-tasks.md` exists and contains:
  ...
  - A dependency graph (text or ASCII) showing task ordering and parallelization opportunities
...
- Dependencies are correct — no task references work from a task it does not depend on
- Dependencies form a DAG (no circular dependencies)
- Tasks are appropriately sized (not too large, not too granular)
- Integration tasks exist and are correctly ordered after the components they integrate
- No task is missing deliverables or acceptance criteria

Up to 3 review rounds. Save findings to `tasks-review.md`. After the final round, present artifacts and any unresolved findings to the user for approval. In autonomous mode: if no findings remain, advance. If unresolved findings remain after max rounds, advance with findings saved under an `## Unresolved` section in `tasks-review.md`.
...
3. If square passes, the work is ready for implementation. kerf's job ends here — implementation uses whatever tooling the team prefers.
...
A work governed by the `plan` jig contains these files:
...
The `{component}` placeholder expands to one entry per component identified during Pass 3. For example, a work with components `parser`, `resolver`, and `emitter` produces:

```
04-research/parser/findings.md
04-research/resolver/findings.md
04-research/emitter/findings.md
05-specs/parser-spec.md
05-specs/resolver-spec.md
05-specs/emitter-spec.md
```
